...
from panav.conflict import plan_obs_conflict, path_conflicted_static_obs
class SAMP_Base:
    '''
    ...
    def plan(self,obstacle_trajectories=[],active_obstacles=[],lazy = True):
        if not lazy:
            return self.plan_plain(obstacle_trajectories=obstacle_trajectories,
                                   active_obstacles=active_obstacles,
                                   active_static_obstacles=self.env.obstacles)
        else:
    ...
    def lazy_plan(self, obstacle_trajectories=[], return_all=True, 
        ...
        i = 0
        K_min = 1
        while i<=m and len(static_obs)<=len(self.env.obstacles):
            print(f'num static obstacles {len(static_obs)}/{len(self.env.obstacles)}')
            print("num dynamic obstacles:{}/{}".format(len(dynamic_obs),m))
            p = self.plan_plain(active_obstacles=dynamic_obs,
                                active_static_obstacles=static_obs,
                                K_min=K_min)
            ...
            new_dynamic_obs = plan_obs_conflict((ts,xs), obstacle_trajectories, self.bloating_r, 
                                               segments_only=True, return_all=return_all)
            
            new_static_obs = []
            if lazy_static:
                new_static_obs = path_conflicted_static_obs(self.env,xs,self.bloating_r,return_all=return_all)
            
            print('new_dynamic_obs',new_dynamic_obs,'new_static_obs',new_static_obs)
            if not new_dynamic_obs and not new_static_obs:
                return p
            
            K_min = len(p[0])-1 # len(p[0])-1 is the latest K value.
            # print("K_min",K_min)

            dynamic_obs += new_dynamic_obs
            static_obs += new_static_obs
    ...
    def plan_plain(self,
        ...
        while self.K<=self.K_max:
            p = self.plan_core(obstacle_trajectories=obstacle_trajectories,
                               active_obstacles=active_obstacles,
                               active_static_obstacles=active_static_obstacles)
            # print(K,p)
            if p:
    ...
    def plan_core(self,obstacles_trajectories=[],
                  active_obstacles=[],
                  active_static_obstacles = []):
            '''
...
class Simple_MILP_Planning(SAMP_Base):
    ...
    def plan_core(self,
        ...
        obs = [([],O) for O in active_static_obstacles] + obs
        lb_active = []
        ub_active = []
...
class Tube_Planning(SAMP_Base):
    ...
    def plan_core(self,obstacle_trajectories=[], 
                  active_obstacles=[], # active dynamic obstacles
                  active_static_obstacles = [],
                  solve_inplace = True):
        '''
        ...
        static_obs = []
        static_obs = active_static_obstacles
        print('len(static_obs)',len(static_obs))

        # Decision variables and standard constraints for the path planning problem.
        t, x, constraints = self.Standard_Tube_Var_Constraints(self.start,self.goal,tubes,static_obs,self.K)
    ...
    def Standard_Tube_Var_Constraints(self,start,goal,tube_obs,static_obs,K):
...
class Path_Tracking(Tube_Planning):
    ...
    def plan_core(self, 
        ...
        t,x,constraints,_ = super().plan_core(obstacle_trajectories=obstacle_trajectories, 
                                              active_obstacles=active_obstacles, 
                                              active_static_obstacles=active_static_obstacles,
                                              solve_inplace=False)
